...
void ProtocolGame::spectate(const std::string& name, const std::string& password)
{
	...
	//words that shows on screen
	ss << "Espectador(es) assistindo " << player->getName() << ": " << (player->client->list().size()) << "\nTempo de Cast: ";
	if (hours > 0)
	{
		ss << hours << " hora(s)";
	}
	...
	if (seconds > 0)
	{
		ss << seconds << " segundo(s)";
	}
	ss << "\nVoce pode trocar de canal usando [CTRL + SETAS]";
