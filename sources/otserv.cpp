...
bool argumentsHandler(StringVec args)
{
	...
	usePostgreSQL = "yes";
		...
		if((*it) == "--version" || (*it) == "-v")
		{
			std::clog << "The " << SOFTWARE_NAME << " Versao: (" << SOFTWARE_VERSION << "." << MINOR_VERSION << ") - Codinome: (" << SOFTWARE_CODENAME << ")\n"
			"Compilado com " << BOOST_COMPILER << " para arquitetura "
			...
			"\n"
			"\t\tFeitos nessa distribuicao:\n"
				
				"\t\tVersao do lua: " << LUA_RELEASE << "\n" <<
				"\t\tVersao do XML: " << LIBXML_DOTTED_VERSION << "\n" <<
				"\t\tVersao do BOOST: " << BOOST_LIB_VERSION << "\n" <<
				"\t\tLuaJIT: " << luajit << "\n" <<
				"\t\tGroundCache: " << groundCache << "\n" <<
				"\t\tServerDiag: " << serverDiag << "\n" <<
				"\t\tLoginServer: " << loginServer << "\n" <<
				"\t\tUsar MySQL: " << useMySQL << "\n" <<
				"\t\tUsar SQLite: " << useSQLite << "\n" <<
				"\t\tUsar PostgreSQL: " << usePostgreSQL << "\n\n" <<
				

			"TFS desenvolvido por: " SOFTWARE_DEVELOPERS ".\n"
			"Visite para atualizacoes, suporte e outros recursos: " GIT_REPO "\n";
			"atualizacoes pedidas na publicacao serao adicionadas em: " GIT_REPO2 "\n";
			return false;
		}
...
{
	puts("Alocacao Falhou, o Servidor usou toda a sua memoria!\nDiminua o tamanho do seu mapa ou compile no modo 64-Bit.");
	OTSYS_getch();
	std::exit(-1);
}
...
int main(int argc, char* argv[])
{
	...
	{
		std::clog << ">> " << g_config.getString(ConfigManager::SERVER_NAME) << " Servidor Ativo!" << std::endl << std::endl;
		servicer.run();
	}
	else
		std::clog << ">> " << g_config.getString(ConfigManager::SERVER_NAME) << " Servidor nao foi ativo! os servicos nao estarao disponiveis..." << std::endl << std::endl;
...
void otserv(StringVec, ServiceManager* services)
{
	...
	if(!getuid() || !geteuid())
	{
		std::clog << "> Aviso: " "O " << SOFTWARE_NAME << " foi executado com superusuario! e eh "
			<< "recomendado abrir como um usuario normal." << std::endl << "Continuar? (y/N)" << std::endl;
	...
	std::clog << "O " << SOFTWARE_NAME << " Versao: (" << SOFTWARE_VERSION << "." << MINOR_VERSION << ") - Codinome: (" << SOFTWARE_CODENAME << ")" << std::endl
		<< "Compilado com " << BOOST_COMPILER << " para arquitetura "
		#if defined(__amd64__) || defined(_M_X64)
		"64 Bits"
		#elif defined(__i386__) || defined(_M_IX86) || defined(_X86_)
		...
		"unk"
			...
			"\t\tFeitos nessa distribuicao: "<< std::endl << std::endl <<
				
				"\t\tVersao do Lua: " << LUA_RELEASE << "\n" <<
				"\t\tVersao do XML: " << LIBXML_DOTTED_VERSION << "\n" <<
				"\t\tVersao do BOOOST: " << BOOST_LIB_VERSION << "\n" <<
				"\t\tLuaJIT: " << luajit << "\n" <<
				"\t\tGroundCache: " << groundCache << "\n" <<
				"\t\tServerDiag: " << serverDiag << "\n" <<
				"\t\tLoginServer: " << loginServer << "\n" <<
			...
			"TFS desenvolvido por: " SOFTWARE_DEVELOPERS "." << std::endl <<
			"Visite para atualizacoes, suporte e outros recursos: " GIT_REPO << std::endl <<
			"Atualizacoes pedidas na publicacao serao adicionadas em: " GIT_REPO2 << std::endl;
	...
	if(!debug.empty())
		std::clog << ">> Depurando:" << debug << "." << std::endl;

	std::clog << ">> Carregando config (" << g_config.getString(ConfigManager::CONFIG_FILE) << ")" << std::endl;
	if(!g_config.load())
	...
	if(g_config.getBool(ConfigManager::DAEMONIZE))
	{
		...
		if(fork())
		{
			std::clog << "sucesso, bye!" << std::endl;
			exit(0);
		}
		else
			std::clog << "falhou, continuando." << std::endl;
	}
	...
	std::clog << ">> Carregando tipo de Seguranca da senha:" << std::endl;
	...
	if(encryptionType == "sha1")
	{
		...
		std::clog << ">>> Usando metodo (SHA1) ... (feito)." << std::endl;
	}
	else
	{
		g_config.setNumber(ConfigManager::ENCRYPTION, ENCRYPTION_PLAIN);
		std::clog << ">>> Usando metodo (PLAINTEXT) ... (feito)" << std::endl << std::endl
			<< ">>> Aviso: Esse metodo eh completamente inseguro!" << std::endl
			<< ">>> Por favor coloque o encryptionType = \"sha1\" (ou qualquer outro metodo disponivel) in config.lua" << std::endl;
		boost::this_thread::sleep(boost::posix_time::seconds(15));
	}

	std::clog << ">> Carregado chave RSA" << std::endl;
	...
	if(db && db->connect())
	{
		std::clog << ">> Iniciando Database Manager" << std::endl;
		if(DatabaseManager::getInstance()->isDatabaseSetup())
		{
			...
			do
			{
				...
				std::clog << "> A database foi atualizada para a versao: " << version << "." << std::endl;
			}
		...
		if(g_config.getBool(ConfigManager::OPTIMIZE_DATABASE) && !DatabaseManager::getInstance()->optimizeTables())
			std::clog << "> Nenhuma tabela foi otimizada." << std::endl;
	}
	else
		startupErrorMessage("Nao foi possivel estabelecer conexao com a database!");

	std::clog << ">> Checando items duplicados" << std::endl;
	...
	if((result = db->storeQuery(query.str())))
	{
			...
			if(serial != "" && serial.length() > 1)
			{
				...
				if((result_ = db->storeQuery(query_playeritems.str())))
				{
					...
					do
					{
						...
						std::clog << ">> Deletado item de 'player_items' com o SERIAL: [" << serial.c_str() << "] Jogador: [" << result_->getDataInt("player_id") << "] Nome do Jogador: [" << name.c_str() << "] ITEM: [" << result_->getDataInt("itemtype") << "] Quant: [" << result_->getDataInt("count") << "]" << std::endl;
						std::ostringstream logText;
						logText << "Deletando item de 'player_items' com o SERIAL: [" << serial << "] Jogador: [" << result_->getDataInt("player_id") << "] Nome do Jogador: [" << name << "] ITEM: [" << result_->getDataInt("itemtype") << "] Quant: [" << result_->getDataInt("count") << "]";
				...
				if((result_ = db->storeQuery(query_playerdepotitems.str())))
				{
					...
					do
					{
						std::string name;
						IOLoginData::getInstance()->getNameByGuid((uint32_t)result_->getDataInt("player_id"), name, false);
						std::clog << ">> Deletado item de 'player_depotitems' com o SERIAL: [" << serial.c_str() << "] JOGADOR: [" << result_->getDataInt("player_id") << "] Nome do Jogador: [" << name.c_str() << "] quant: [" << result_->getDataInt("itemtype") << "] COUNT: [" << result_->getDataInt("count") << "]" << std::endl;
						std::ostringstream logText;
						logText << "Deletado item de 'player_depotitems' com o SERIAL: [" << serial << "] JOGADOR: [" << result_->getDataInt("player_id") << "] Nome do Jogador: [" << name << "] ITEM: [" << result_->getDataInt("itemtype") << "] quant: [" << result_->getDataInt("count") << "]";
						Logger::getInstance()->eFile("anti_dupe.log", logText.str(), true);
					}
					while(result_->next());
					result_->free();
				...
				if(!db->query(query_deletepi.str()))
					std::clog << ">> Nao foi possivel deletar items duplicados de 'player_items'!" << std::endl;
				...
				if(!db->query(query_deletedi.str()))
					std::clog << ">> Nao foi possivel deletar items duplicados de 'player_depotitems'!" << std::endl;
				...
				if(!db->query(query_deleteti.str()))
					std::clog << ">> Nao foi possivel deletar items duplicados de 'tile_items'!" << std::endl;
		...
		if(duplicated)
			std::clog << ">> Items duplicados foram removidos com sucesso." << std::endl;
	} else {
		std::clog << ">> Nao ha items duplicados no servidor." << std::endl;
	}
	...
	{
		std::clog << "Impossivel carregar items (XML)! Continuar? (y/N)" << std::endl;
		char buffer = OTSYS_getch();
		if(buffer != 121 && buffer != 89)
			startupErrorMessage("Impossivel carregar items (XML)!");
	}

	std::clog << ">> Carregando Cargos (groups)" << std::endl;
	if(!Groups::getInstance()->loadFromXml())
		startupErrorMessage("Impossivel carregar groups!");

	std::clog << ">> Carregando Roupas(outfits)" << std::endl;
	if(!Outfits::getInstance()->loadFromXml())
		startupErrorMessage("Impossivel carregar outfits!");

	std::clog << ">> Carregando Missoes(quests)" << std::endl;
	if(!Quests::getInstance()->loadFromXml())
		startupErrorMessage("Impossivel carregar quests!");

	std::clog << ">> Carregando ataques(raids)" << std::endl;
	if(!Raids::getInstance()->loadFromXml())
		startupErrorMessage("Impossivel carregar raids!");

	std::clog << ">> Carregando Vocacoes" << std::endl;
	if(!Vocations::getInstance()->loadFromXml())
		startupErrorMessage("Impossivel carregar vocations!");

	std::clog << ">> Carregando Bate-Papo" << std::endl;
	if(!g_chat.loadFromXml())
		startupErrorMessage("Impossivel carregar channels!");

	std::clog << ">> Carregando Stages" << std::endl;
	if(!g_game.loadExperienceStages())
		startupErrorMessage("Impossivel carregar estagios de experiencia (stages)!");
	...
	{
		std::clog << ">> Carregando Sistemas de Scripts:" << std::endl;
	...
	#ifdef __LOGIN_SERVER__
	std::clog << ">> Carregando os Servidores do Jogo" << std::endl;
	if(!GameServers::getInstance()->loadFromXml(true))
		startupErrorMessage("Impossivel carregar os Servidores do jogo!");
	#endif

	std::clog << ">> Carregando Monstros" << std::endl;
	if(!g_monsters.loadFromXml())
	{
		std::clog << "Impossivel carregar os monstros! Continuar? (y/N)" << std::endl;
		char buffer = OTSYS_getch();
		if(buffer != 121 && buffer != 89)
			startupErrorMessage("Impossivel carregar monsters!");
	}
	...
	if(fileExists(getFilePath(FILE_TYPE_OTHER, "npc/npcs.xml")))
	{
		...
		{
			std::clog << "Impossivel carregar npcs! Continuar? (y/N)" << std::endl;
			char buffer = OTSYS_getch();
			if (buffer != 121 && buffer != 89)
				startupErrorMessage("Impossivel carregar npcs!");
		}
	}

	std::clog << ">> Carregando Raids" << std::endl;
	...
	std::clog << ">> Checando o tipo de mundo... ";
	...
	{
		std::clog << std::endl;
		startupErrorMessage("Tipo de mundo desconhecido: " + g_config.getString(ConfigManager::WORLD_TYPE));
	}

	std::clog << ">> Comecando a dominar o mundo... (feito)." << std::endl;

	std::clog << ">> Iniciando o jogo e ligando os servicos:" << std::endl;
	...
	if (resolvedIp == INADDR_NONE) {
		...
		if (!he) {
			std::ostringstream ss;
			ss << "ERRO? nao foi possivel resolver o IP: " << ip << "!" << std::endl;
	...
	std::clog << std::endl << ">> Tudo parece bem, o servidor esta iniciando..." << std::endl;
